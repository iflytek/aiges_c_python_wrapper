...
PYBND11_MODULE(bar, m) {
	 py::class_<Vehiclee> foo = (py::class_) py::module::import("simpleFuncs").attr("Vehiclee");
	 foo.def(py::init<>())
          .def_readwrite("id",&Vehiclee::vehicle_id);
}
...
int main() {
  ...
  Vehiclee *v2 = ret.cast<Vehiclee *>();

  std::cout << "affter" << v2->vehicle_id << std::endl;
  return 0;
}
