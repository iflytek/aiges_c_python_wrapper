            ...
            if (rltSize != 0)
            {
                ...
                for (int idx = 0; idx < rltSize; idx++)
                {
                    ...
                    if (ret != 0)
                    {
                        ...
                        std::cout<<"转换后的字符串"<<tmpRltKey<<"长度"<<strlen(tmpRltKey.data())<<std::endl;
                        tmpData->key = (char *)malloc(strlen(tmpRltKey.data()));
	                    memcpy(tmpData->key,(char*)tmpRltKey.data(), strlen(tmpRltKey.data()));
