...
#   elif !defined(_PTHREADS) && defined(_REENTRANT)
...
#       define _PTHREADS
#   endif
# endif // defined(FM_NOTHREADS)

# if !defined(_PTHREADS) && !defined(_WIN32THREADS) && !defined(FM_NOTHREADS)
#	define _NOTHREADS
#	error ("No thread model defined, continued with FM_NOTHREADS")
# endif
...
#       endif
	{
		...
		::pthread_mutex_init(&_M_mtx_impl, &mtx_attr);
...
#       endif
	...
	~fast_mutex()
	{
		_FAST_MUTEX_ASSERT(_M_locked_thrid == -1, "~fast_mutex(): still locked");
#       if _FAST_MUTEX_CHECK_INITIALIZATION
		_M_initialized = false;
#       endif
...
#       ifdef _DEBUG
		_FAST_MUTEX_ASSERT(_M_locked_thrid == -1 ||
			_M_locked_thrid == ::GetCurrentThreadId(),
			"lock(): already locked");
		_M_locked_thrid = ::GetCurrentThreadId();
#       endif
	}
...
#       endif
#       ifdef _DEBUG
		_FAST_MUTEX_ASSERT(_M_locked_thrid != -1, "unlock(): not locked");
		if (_M_mtx_impl.RecursionCount == 1)
			_M_locked_thrid = -1;
#       endif
...
#       endif
	...
	{
#       ifdef _DEBUG
		_FAST_MUTEX_ASSERT(!_M_locked, "trylock(): already locked");
		_M_locked = true;
#       endif
		return true;
	}
